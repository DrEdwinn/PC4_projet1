...
B = 2*M + sparse(toeplitz([-2*lambda^2-6*mu^2 lambda^2+4*mu^2 -mu^zeros(1,N-4)]));
...
xlabel('t'); ylabel('u'); title('Stiff String Equation: FD Output (left)'); subplot(2,1,2); plot([0:NF-1]*k, out(:,2), 'k');
